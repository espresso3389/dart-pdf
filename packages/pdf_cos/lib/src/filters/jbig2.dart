...
class Jbig2Decoder {
  ...
  void _readGenericRegionSegment(Uint8List data) {
    ...
    if (mmr == 1) {
      ...
      final decoder = _MqDecoder(payload);
  ...
  List<_Bitmap> _readSymbolDictionary(Uint8List data, List<int> referred) {
    ...
    final decoder = _MqDecoder(Uint8List.sublistView(data, p));
    final contexts = Int8List(1 << 16);
    final indexes = Uint8List(1 << 16);
    final iadh = _IntContext();
    final iadw = _IntContext();
    final iaex = _IntContext();
  ...
  void _readTextRegion(Uint8List data, List<int> referred) {
    ...
    final decoder = _MqDecoder(Uint8List.sublistView(data, p));
    final iadt = _IntContext();
    final iafs = _IntContext();
    final iads = _IntContext();
    final iait = _IntContext();
    final iari = _IntContext();
...
class _MqDecoder {
  ...
  int? decodeInt(_IntContext context) {
    var prev = 1;
    int bit() {
